        ...
        {pipelineItems.map(item => (
          <div className="pipeline-card" key={item.title}>
            <div className="card-header">
              <span className="card-num" style={{ color: 'var(--purple)' }}>Pipeline</span>
            ...
            <div className="card-tags">
              {item.tags.map(tag => (
                <span key={tag} className="tag" style={{ borderColor: 'rgba(124,58,237,0.18)', color: 'var(--purple)' }}>
                  {tag}
                </span>
              ))}
            </div>
            {item.detailPath && (
              <Link to={item.detailPath} className="card-view-btn" style={{ marginTop: '1.25rem', background: 'rgba(124,58,237,0.07)', color: 'var(--purple)', borderColor: 'rgba(124,58,237,0.2)' }}>
                View Details →
              </Link>
            )}
          </div>
